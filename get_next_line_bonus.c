...
char	*get_next_line(int fd)
{
	...
	rb = 1;
	if (fd < 0 || BUFFER_SIZE <= 0 || read(fd, "", 0) == -1 || fd > 1024)
		return (NULL);
	line = ft_strinit();
	line = ft_joinandfree(line, buffer[fd]);
	while (ft_strchr(buffer[fd]) == 0 && rb != 0)
	{
		rb = read(fd, buffer[fd], BUFFER_SIZE);
		buffer[fd][rb] = '\0';
		line = ft_joinandfree(line, buffer[fd]);
	}
	if (rb == 0 && line[0] == 0)
